      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {sensoryProtocolSteps.map((item, idx) => {
            const localizedStep = content.steps[idx];
            return (
              <motion.div
                key={item.step}
              ...
              >
                ...
                <h3 className="text-2xl mb-4">{localizedStep.title}</h3>
                <p className="text-sm opacity-70 leading-relaxed mb-6">{localizedStep.desc}</p>

                <div className="pt-4 border-t border-neutral-300">
                  <p className="text-xs font-baskervville-italic opacity-60">{localizedStep.insight}</p>
                </div>
              </motion.div>
            );
          })}
